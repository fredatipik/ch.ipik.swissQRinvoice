...
class CRM_SwissQRInvoice_Page_Duplicate extends CRM_Core_Page {
  public function run() {
    ...
    if (!$id) CRM_Core_Error::fatal(ts('ID manquant.', ['domain' => 'ch.ipik.swissQRinvoice']));
    $new = CRM_SwissQRInvoice_BAO_Invoice::duplicate($id);
    CRM_Core_Session::setStatus(
      ts('Facture dupliquée : %1', ['domain' => 'ch.ipik.swissQRinvoice', 1 => $new['invoice_number']]),
      ts('Succès', ['domain' => 'ch.ipik.swissQRinvoice']), 'success'
    );
