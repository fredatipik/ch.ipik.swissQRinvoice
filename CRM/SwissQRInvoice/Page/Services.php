...
class CRM_SwissQRInvoice_Page_Services extends CRM_Core_Page {
  public function run() {
    CRM_Utils_System::setTitle(ts('Prestations'));
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      if ($action === 'save') {
        ...
        CRM_Core_Session::setStatus(ts('Prestation enregistrée.'), ts('Succès'), 'success');
      }
      if ($action === 'delete') {
        $sid = (int)($_POST['sid'] ?? 0);
        if ($sid) CRM_SwissQRInvoice_BAO_Invoice::deleteService($sid);
        CRM_Core_Session::setStatus(ts('Prestation supprimée.'), ts('Succès'), 'success');
      }
